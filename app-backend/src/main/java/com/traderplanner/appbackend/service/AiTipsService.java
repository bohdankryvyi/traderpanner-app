...
public class AiTipsService {
    ...
    public AiTipsService(AiProperties aiProperties,
    ...
    /**
     * Fetch OVERVIEW for first tickers (alphabetically); take first 3 with data. No EPS/valuation ranking.
     */
    private AiTipsResponse generateTipsWithAlphavantage(Timeframe timeframe, OffsetDateTime now,
        ...
        List<SecurityDto> sorted = securities.stream()
                .sorted(Comparator.comparing(SecurityDto::getTicker))
                .limit(ALPHAVANTAGE_MAX_TICKERS)
                .toList();
    ...
    private AiTipsResponse buildTipsResponse(Timeframe timeframe, OffsetDateTime now, List<SecurityDto> top3) {
        ...
        String text = String.format(Locale.ENGLISH,
                "For the %s, top 3 from whitelist (Alpha Vantage):%n%n" +
                        "1. %s (%s) – %s%n" +
                        "2. %s (%s) – %s%n" +
                        "3. %s (%s) – %s%n%n" +
                        "From S&P 500 whitelist. Watchlist only; not financial advice.",
                horizon,
                s1.getTicker(), s1.getSector(), s1.getCompany(),
                s2.getTicker(), s2.getSector(), s2.getCompany(),
                s3.getTicker(), s3.getSector(), s3.getCompany());

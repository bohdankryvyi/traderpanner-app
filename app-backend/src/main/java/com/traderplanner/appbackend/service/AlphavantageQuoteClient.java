...
/**
 * Fetches GLOBAL_QUOTE from Alpha Vantage for real price. Returns null on failure.
 */
@Component
public class AlphavantageQuoteClient {
    ...
    public BigDecimal fetchPriceUsd(String symbol, String apiKey) {
        ...
        try {
            ...
            if (globalQuote == null || !globalQuote.isObject()) return null;
            JsonNode priceNode = globalQuote.get("05. price");
            if (priceNode == null) return null;
            String priceStr = priceNode.asText();
            if (priceStr == null || priceStr.isBlank() || "None".equalsIgnoreCase(priceStr)) return null;

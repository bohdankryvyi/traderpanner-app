...
public class PortfolioService {
	...
	public PortfolioService(PortfolioPositionRepository repository, SecurityService securityService,
			PriceService priceService, FxProperties fxProperties) {
	...
	public List<PortfolioPositionResponse> getAll() {
		...
		Set<String> tickers = positions.stream().map(PortfolioPosition::getTicker).collect(Collectors.toSet());
		...
		for (PortfolioPosition position : positions) {
			...
			BigDecimal targetUsd = Optional.ofNullable(position.getTargetPrice())
					.map(tp -> toUsd(tp, position.getCurrency())).orElse(priceUsd);
		...
		for (PortfolioPosition position : positions) {
			...
			if (totalInvested.compareTo(BigDecimal.ZERO) > 0) {
				sharePercent = investedUsd.multiply(BigDecimal.valueOf(100)).divide(totalInvested, 4,
						RoundingMode.HALF_UP);
			}

			BigDecimal profitSharePercent = BigDecimal.ZERO;
			if (totalExpectedProfit.compareTo(BigDecimal.ZERO) != 0) {
				profitSharePercent = expectedProfitUsd.multiply(BigDecimal.valueOf(100)).divide(totalExpectedProfit, 4,
						RoundingMode.HALF_UP);
			}
	...
	private PortfolioPositionResponse enrichSingle(PortfolioPosition position) {
		...
		BigDecimal investedUsd = toUsd(position.getBuyPrice(), position.getCurrency()).multiply(position.getQuantity());
		BigDecimal targetUsd = Optional.ofNullable(position.getTargetPrice())
				.map(tp -> toUsd(tp, position.getCurrency())).orElse(priceUsd);
		BigDecimal expectedUsd = targetUsd.multiply(position.getQuantity());
		BigDecimal expectedProfitUsd = expectedUsd.subtract(investedUsd);
	...
	private BigDecimal computeUpside(BigDecimal currentPriceUsd, BigDecimal targetUsd) {
		...
		return targetUsd.subtract(currentPriceUsd).multiply(BigDecimal.valueOf(100)).divide(currentPriceUsd, 4,
				RoundingMode.HALF_UP);
	}
}
